    ...
    async def send_priority_miners(self, priority_miners):
        ...
        if nats_url:
            try:
                ...
                logger.info(f"An error occurred when connecting to nats 🚩 {str(e)}")
                logger.debug(
                    f"Failed to connect to NATS server ( {nats_url} ) : {str(e)}"
                )
                return
            ...
            try:
                ...
                logger.info(
                    f"Error publishing priority miners message to NATS ({nats_url})"
                )
                logger.debug(f"Error publishing priority miners to NATS: {str(e)}")
